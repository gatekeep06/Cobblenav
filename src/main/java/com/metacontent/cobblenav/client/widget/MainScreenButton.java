...
public class MainScreenButton extends AbstractPokenavButton {
    private final MutableText message;
    private final int pullout;
    private final OnPressed action;
    private boolean isSoundPlayed = false;

    public MainScreenButton(int x, int y, int width, int height, int offsetX, int offsetY, int pullout, Text text, OnPressed action) {
    ...
    protected void renderButton(DrawContext drawContext, int i, int j, float f) {
        ...
        drawScaledText(drawContext, FONT, message,
                getX() + width / 2 + (isHovered() ? 0 : pullout), getY() + height / 2 - 4, 1, 1, (int) (getWidth() * 0.75), 0, true, false, i, j);
    }
